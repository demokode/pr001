...
- (void)bindObject:(id)obj toColumn:(int)idx inStatement:(sqlite3_stmt*)pStmt {
    ...
    // FIXME - someday check the return codes on these binds.
        ...
        if (self.hasDateFormatter)
            sqlite3_bind_text(pStmt, idx, [[self stringFromDate:obj] cStringUsingEncoding:NSNonLossyASCIIStringEncoding], -1, SQLITE_STATIC);
        else
        ...
        if (strcmp([obj objCType], @encode(BOOL)) == 0) {
            ...
            sqlite3_bind_text(pStmt, idx, [[obj description] cStringUsingEncoding:NSNonLossyASCIIStringEncoding], -1, SQLITE_STATIC);
        }
    }
    else {
        sqlite3_bind_text(pStmt, idx, [[obj description] cStringUsingEncoding:NSNonLossyASCIIStringEncoding], -1, SQLITE_STATIC);
        ...
        if (last == '%') {
            switch (current) {
                ...
                case 'c':
                    ...
                    break;
                case 's':
                    arg = [NSString stringWithCString:va_arg(args, char*) encoding:NSNonLossyASCIIStringEncoding];
                    break;
                case 'd':
                case 'D':
                case 'i':
...
- (FMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (!pStmt) {
        ...
        rc      = sqlite3_prepare_v2(_db, [sql cStringUsingEncoding:NSNonLossyASCIIStringEncoding], -1, &pStmt, 0);
